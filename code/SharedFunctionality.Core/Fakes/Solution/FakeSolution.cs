        ...
        private static string GetProjectConfigurationTemplate(string platform, string appmodel, string language, string projectRelativeToSolutionPath, bool isCPSProject, bool hasPlatforms)
        {
            switch (platform)
            {
                case Platforms.Avalonia:
                    ...
                    {
                        return File.ReadAllText(@"Fakes\Solution\ProjectConfigurationTemplates\Wpf\WpfCoreProjectTemplate.txt");
                    }
                    else if (projectRelativeToSolutionPath.Contains("XamlIsland"))
                    {
                        return File.ReadAllText(@"Fakes\Solution\ProjectConfigurationTemplates\Wpf\XamlIslandProjectTemplate.txt");
                    }
                    else
                    {
                        if (hasPlatforms)
                        {
                            return File.ReadAllText(@"Fakes\Solution\ProjectConfigurationTemplates\Wpf\WpfProjectTemplate.txt");
                        }
                        else
                        {
